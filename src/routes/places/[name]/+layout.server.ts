export const load = async ({ locals: { supabase, getSession, userConnections }, params }) => {
	...
	if (placeOwner == session?.user.id) {
		return { restaurant, placeRanking, owner: true, placeName: params.name, checkIns, posts, placeId};
	} else {
		return { restaurant, placeRanking, owner: false, placeName: params.name, checkIns, posts };
	}
};
